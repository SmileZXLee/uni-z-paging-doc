...
| refreshToPage <Badge text="2.5.9"/>                | 刷新列表数据至指定页。<p style="color:red;">(必须保证@query绑定的方法中的pageNo和pageSize和传给服务端的一致)</p> | `参数1(必填)`:目标页数，例如目标页数=5时则代表刷新列表至第5页，此时pageNo会变为5，列表会展示前5页的数据<br /><Badge text="2.5.3"/> `返回值(return)`:`Promise(resolve({totalList, noMore}), reject)`获取本次操作请求结束后的【总列表】和【是否没有更多数据】 |
