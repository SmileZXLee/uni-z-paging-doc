...
| refresh <Badge text="2.0.4"/>                      | 刷新列表数据，pageNo和pageSize不会重置，列表数据会重新从服务端获取。<p style="color:red;">(必须保证@query绑定的方法中的pageNo和pageSize和传给服务端的一致)</p> | <Badge text="2.5.3"/> `返回值(return)`:`Promise(resolve({totalList, noMore}), reject)`获取本次操作请求结束后的【总列表】和【是否没有更多数据】 |
